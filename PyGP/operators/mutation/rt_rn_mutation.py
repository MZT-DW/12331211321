...
LastEditTime: 2024-05-21 21:27:05
LastEditors: your name
Description: 
FilePath: \GESR\PyGP\operators\mutation\rt_rn_mutation.py
'''
...
def randSubtree(prog, depth_limit, rand_state, data_rg):
    init_func = prog.funcs.funcSelect(rand_state.randint(0, prog.funcs.len()))
    if depth_limit <= 1:
        if prog.const_range is None:
            terminal = rand_state.randint(0, prog.n_terms)
        else:
    ...
    while stack:
        node = stack.pop()
        childs = []
        if PyGP.INTERVAL_COMPUTE and node.dtype == "Func" and (node.nodeval.name == '/'):
            divide_check.append(node)
        for i in range(node.getArity()):
            if depth + 1 < depth_limit and rand_state.randint(0, funcs_size + prog.n_terms) < funcs_size:
    ...
    for node in divide_check:
        ...
        if (smt_rg[0] <= 0. <= smt_rg[1] or math.fabs(smt_rg[0]) == 0.0 or math.fabs(smt_rg[1]) == 0.0)\
                and ((node.nodeval.name == '/' and childs[0].print_exp_subtree() != childs[1].print_exp_subtree())):
            if node.nodeval.name == '/':
...
def mutation(progs: [Program], smts, mut_rate, funcs):
    ...
    slted_progs = np.squeeze(np.where(np.random.random(len(progs)) < mut_rate))
    ...
    split_progs = slted_progs[r_prob < 0.5]
    split_rnode = rnode[r_prob < 0.5]
    time = 5
    # assert (not cst_r)
    for (key, i) in enumerate(split_progs):
        ...
        count = 0
        while(count < time):
            r_subtr = randSubtree(progs[i], np.random.randint(0, h_max), np.random, data_rg)
    ...
    tmn = list(map(lambda x: np.random.randint(0, len_nterms + 1) if cst_r else np.random.randint(0, len_nterms), split_progs))
    for (key, i) in enumerate(split_progs):
        sub_root = progs[i].getSubTree(split_rnode[key])
        terminal = tmn[key]
        ...
        if sub_root.dtype == 'Func':
            ...
            rd_val = np.random.randint(0, len(func_list) + 1) == len(func_list)
            if not rd_val:
                while(count < time):
                    r_oper = func_list[np.random.randint(0, len(func_list))]
                    ...
                    if PyGP.INTERVAL_COMPUTE and (((r_oper.name == '/') and (child_rg[0][0] <= 0. <= child_rg[0][1] or math.fabs(child_rg[0][0]) == 0.0 or math.fabs(child_rg[0][1]) == 0.0)) or not bounds_check(sub_root, new_rg, data_rg)):
                        sub_root.reset(func_origin)
                    else:
                        break
                    count += 1
        if sub_root.dtype != 'Func' or rd_val:
            if terminal == len_nterms:
                terminal = np.random.uniform(progs[i].const_range[0], progs[i].const_range[1])
