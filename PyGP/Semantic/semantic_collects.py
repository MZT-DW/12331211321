    ...
    def compute_tg(self, prog_id):
        ...
        for i in range(self.semantics[prog_id].count):
            ...
            if posi not in posis:
                tgs.append(i)
                posis.append(posi)
                if not (np.isnan(tgsmt)).any() and not (tgdrvt == 0).all():
                    # resmax = resval
                    #
                    # tgdrvt = PyGP.abs_normalize(tgdrvt)
                    vec = np.subtract(tgsmt, cdd)
                    size = len(self.get_tg_node(prog_id, i))
                    # print(size, math.e ** (size / (PyGP.MAX_TREE_SIZE * 2)))
                    # resval = np.sqrt(np.dot(vec * tgdrvt, vec * tgdrvt)) * (1 + size / (PyGP.MAX_TREE_SIZE * 2))
                    resval = np.sqrt(np.dot(vec * tgdrvt, vec * tgdrvt))
                    if resval > res_min:
                        res_min = resval
                        res_id = i
